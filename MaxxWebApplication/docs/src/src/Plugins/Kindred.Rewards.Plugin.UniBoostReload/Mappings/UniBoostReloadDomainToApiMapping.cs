...
namespace Kindred.Rewards.Plugin.UniBoostReload.Mappings;

public class UniBoostReloadDomainToApiMapping : Profile
{
    public UniBoostReloadDomainToApiMapping()
    {
        CreateMap<RewardDomainModel, UniBoostReloadRewardResponse>(MemberList.None)
            ...
            .AfterMap(
                (s, d) =>
                {
                    d.RewardParameters = (UniBoostReloadParametersApiModel)Extensions.Extensions.MapFromRewardTermsToRewardParameters(s.Terms, s.Type);
                });
    }
}
