...
public class UniBoostReloadApiToDomainMapping : Profile
{
    public UniBoostReloadApiToDomainMapping()
    {
        CreateMap<UniBoostReloadRewardRequest, RewardDomainModel>(MemberList.None)
            .ForMember(d => d.CustomerId, opt => opt.MapFrom(s => s.CustomerId))
            .ForMember(d => d.Name, opt => opt.MapFrom(s => s.Name))
            .ForMember(d => d.Comments, opt => opt.MapFrom(s => s.Comments))
            .ForMember(d => d.Category, opt => opt.MapFrom(_ => RewardCategory.Boost))
            .ForMember(d => d.Type, opt => opt.MapFrom(s => RewardType.UniboostReload))
            ...
            .AfterMap(
                (request, model) =>
                {
                    model.Terms.RewardParameters = Extensions.Extensions.MapFromRewardParametersToDictionary(request.RewardParameters, RewardType.UniboostReload.ToString());
                    model.Terms.Restrictions.Reload =
                        Extensions.Extensions.MapFromRewardParametersToRewardReload(request.RewardParameters, RewardType.UniboostReload.ToString());
