@Acceptance @variants @maxExtraWinnings @profitboost @ignore
Feature: ProfitboostWithMaxExtraWinningsConfigured

Background:
    Given I have bonuses in the system
        | name              | rewardType  | legTable                    | allowedFormulae | customer_id | minStages | maxStages | minCombinations | maxCombinations | maxExtraWinnings |
        | profitboost_bonus | profitboost | {"1":"10","2":"10","4":"0"} | singles,doubles | 918792      | 1         | 15        | 1               | 2               | 1                |
...
Scenario: Claim with 2 stages where the reward payment is above the configured max extra winnings
    When I submit ClaimEntitlement request for following criteria for Currency 'AUD':
        ...
        | 918792      | profitboost_bonus | 1                | 1.5,25.69      |
    Then the HttpStatusCode should be 200
    ...
    And I expect the correct Payout with rewardPaymentAmount of 1

Scenario: Claim with 2 stages where the reward payment is below the configured max extra winnings
    When I submit ClaimEntitlement request for following criteria for Currency 'AUD':
        | customer_id | name              | number_of_claims | stagePriceData |
        | 918792      | profitboost_bonus | 1                | 1,3.1          |
    Then the HttpStatusCode should be 200
    And I expect following in the ClaimedEntitlements for the customerID='918792':
        | name              |
        | profitboost_bonus |
    When I settle the bet for following claim
        ...
        | profitboost_bonus | Winning     | 3.00         | false             | 918792      |
    Then I expect to receive RewardClaimSettledEvent
    And I expect the correct Payout with rewardPaymentAmount of 0.63
