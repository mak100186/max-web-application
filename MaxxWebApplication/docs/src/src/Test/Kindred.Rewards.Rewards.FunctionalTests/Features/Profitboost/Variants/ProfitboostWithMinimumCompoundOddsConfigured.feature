@Acceptance @variants @minimumcompoundodds @profitboost @ignore
Feature: ProfitboostBonusWithMinimumCompoundOddsConfigured
...
Background:
    ...
    Given I have bonuses in the system
        | name              | rewardType  | legTable                    | allowedFormulae | customer_id | minStages | maxStages | minCombinations | maxCombinations | minimumCompoundOdds |
...
Scenario: Claim with 1 stage that is above the configured minimum compound odds
...
Scenario: Claim with 1 stage that is below the configured minimum compound odds
    When I submit ClaimEntitlement request for following criteria for Currency 'AUD':
        | customer_id | name              | number_of_claims | stagePriceData |
    ...
    When I settle the bet for following claim
        ...
        | profitboost_bonus | Winning     | 3.00         | false             | 918792      |
    Then the HttpStatusCode should be 400
    And Response should contain error 'Cumulative odds fall below configured minimum: 1.5'
