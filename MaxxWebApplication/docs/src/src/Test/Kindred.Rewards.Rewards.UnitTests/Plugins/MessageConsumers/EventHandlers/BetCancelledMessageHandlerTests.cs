...
using BetCancelled = Kindred.Rewards.Plugin.MessageConsumers.EventHandlers.BetCancelled;

namespace Kindred.Rewards.Rewards.UnitTests.Plugins.MessageConsumers.EventHandlers;

[TestFixture]
[Category("Unit")]
public class BetCancelledMessageHandlerTests
{
    private BetCancelledMessageHandler _subject;
    private Mock<IClaimService> _claimService;

    [SetUp]
    public void SetUp()
    {
        var mockLogger = new Mock<ILogger<BetCancelledMessageHandler>>();
    ...
    [Test]
    public void ProcessMessage_DelegatesControlToRelevantService_WhenBetIsCancelled()
    {
        //arrange
        Fixture f = new();
        var betMessage = f.Create<BetCancelled>();
        betMessage.Resources.First().Status = BetStatus.Cancelled;

        //act
        _subject.ProcessMessage(new Message<string, BetCancelled>
        {
            Headers = new()
            {
    ...
    [Test]
    public void ProcessMessage_DelegatesControlToRelevantService_WhenBetIsNotCancelled()
    {
        //arrange
        Fixture f = new();
        var betMessage = f.Create<BetCancelled>();
        betMessage.Resources.First().Status = f.Create<Generator<BetStatus>>().First(s => BetStatus.Cancelled != s);
